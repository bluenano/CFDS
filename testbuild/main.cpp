...
void DoSomethingWithResults(const FrameResults& r/*, any other params you need for impl*/)
{
    ...
    pqxx::result r1 = txn.exec(
        ...
        txn.quote(video_id) + ", " +
        txn.quote(r.frameno) + ", " +
        txn.quote(r.right_pupil.x16) + ", " +
        txn.quote(r.right_pupil.y16) + ", " +
        txn.quote(r.left_pupil.x16) + ", " +
        txn.quote(r.left_pupil.y16) + ", " +
        txn.quote(r.rotation.roll) + ", " +
        txn.quote(r.rotation.pitch) + ", " +
        txn.quote(r.rotation.yaw) +
        ") RETURNING frameid"
    );
    ...
    for (int i=0;i<68;i++) {
        pqxx::result r2 = txn.exec(
            ...
            "VALUES (" +
            txn.quote(i + 1) + ", " +
            txn.quote(r.marks68[i].x16) + ", " +
            txn.quote(r.marks68[i].y16) + ", " +
            txn.quote(frameid) +
            ")"
        );
    }
